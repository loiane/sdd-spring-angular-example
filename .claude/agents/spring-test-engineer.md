...
3. **Write** the smallest test that asserts the AC. The annotation block on every `@Test` method is **exactly two** lines, in this order: `@Test` → `@DisplayName("given <precondition>, when <action>, then <outcome>")`. Always use pure Given/When/Then format — no AC prefix. Do **not** use `@Tag`.
...
- **Every `@Test` and `@ParameterizedTest` method must carry `@DisplayName`.** No exceptions for "small" or "obvious" tests. Format: `"given <precondition>, when <action>, then <outcome>"` — pure Given/When/Then, no AC prefix. Add the annotation in the red phase *before* the test body; audit again in the simplify phase to catch any newly-authored tests that slipped through. Pre-existing legacy tests in the same file are NOT retroactively rewritten — only newly-authored or modified tests are in scope.
- **Never use `@Tag`.** AC traceability is maintained at the task level in `04-tasks.md` (`AC-IDs` field) and `.tdd-state.json` (`acs_covered` field) — not via test annotations.
