...
- **No `FooService` interface + `FooServiceImpl` pair when a single implementation exists and no other feature depends on the interface** — flag as `major`; the fix is to collapse into a single concrete `@Service` class named `FooService`.
