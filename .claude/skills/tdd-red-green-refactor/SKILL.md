...
- Write the smallest possible test that asserts the behavior described by the AC.
  - **Annotation block on every `@Test` method is non-negotiable: `@Test` → `@DisplayName("...")`.** Write the `@DisplayName` *before* the test body — the act of articulating the display name forces clarity about what the test verifies. Format: `"given <precondition>, when <action>, then <outcome>"` — pure Given/When/Then, no AC prefix. Do **not** use `@Tag`; AC traceability is tracked at the task level in `04-tasks.md`.
...
- **Audit every newly-authored or modified test method in the diff for `@DisplayName`.** Any `@Test` or `@ParameterizedTest` without one gets it added here using Given/When/Then format. This is the safety net for the red-phase rule — if a test slipped through without it, simplify is the last chance to catch it before the task is `done`. Also remove any `@Tag` that slipped in. Pre-existing legacy tests in the same file that were not touched by this task are NOT retroactively rewritten (scope-creep guard).
