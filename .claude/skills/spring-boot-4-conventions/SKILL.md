...
- Controller input constraints (`@Positive`, `@Max`, `@Valid`, etc.) without corresponding tests — every constraint must have at least one test that fires it.
- `FooService` interface + `FooServiceImpl` class when there is only one implementation — use a single concrete `@Service` class named `FooService` directly in the feature package. No interface, no `Impl` suffix. Reserve a service interface only when another feature's code must depend on it (e.g. a cross-bounded-context port). Unnecessary interfaces add files, inflate tasks, require `@InjectMocks` to reference `FooServiceImpl`, and convey false intent.
